    ...
    public function list()
    {
        ...
        foreach (\Yao\Facade\Route::getRoute() as $method => $routes) {
            foreach ($routes as $route => $locate) {
                ...
                echo '|' . $this->_format(strtoupper($method), 6) . '|' . $this->_format($route, 46) . '|' . $this->_format($locate['route'], 50) . '| ' . $this->_format(\Yao\App::instance()->make('alias')->getAliasByUri($route), 15) . "|\n";
            }
        }
        exit(self::SEPARATOR);
    }
